...
{
    public class LoginCommandHandler(IGenericRepository<Shared.Entites.User> _repository , IAuthService _authService) : IRequestHandler<LoginCommand, ResponseResult<AuthModel>>
    {
        public async Task<ResponseResult<AuthModel>> Handle(LoginCommand request, CancellationToken cancellationToken)
        {
            var user =await _repository.GetByCriteriaAsync(u => u.Email == request.Email);
            if (user == null) 
